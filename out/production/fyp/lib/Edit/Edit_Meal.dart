...
import 'package:medichat/Add/Add_Nutrients.dart';
...
class EditMealScreen extends StatefulWidget {
  @override
  State<EditMealScreen> createState() => _SetEditMealScreenState();
}

class _SetEditMealScreenState extends State<EditMealScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      setState(() {
        ...
        calorieE = arguments['calorie'] ?? 0;
        selectedMeal = title;
        oldRecord = mealTitle;

        date = TextEditingController(text: dateD);
        food = TextEditingController(text: mealTitle);
        descr = TextEditingController(text: descrE);
        calorie = TextEditingController(text: calorieE.toString());
      });

      // Fetch nutrient data
      _getNutrientsData().then((_) {
        setState(() {
          isDataLoaded = true;  // Set the flag after loading
        });
      });
    });
  }
    ...
    return Scaffold(
      ...
      body: isDataLoaded
        ...
        child: Container(
                    ...
                    SizedBox(
                      ...
                      child: RoundedButton(
                        ...
                        onPressed: () {
                          ...
                          _AddMeal(); // Proceed if all inputs are valid
                        },
                      ),
                    ),
                  ],
          ...
          : Center(child: CircularProgressIndicator()), // Show a loading indicator while data loads
    );
  }
  ...
  Widget _buildMealRadioButton(String meal) {
    return GestureDetector(
      onTap: () {
        // Only update the selectedMeal if the boolean flag is false (editable state)
        if (!isReadOnly) {
          setState(() {
            selectedMeal = title;
          });
      ...
      child: AbsorbPointer(  // This will prevent interaction when in read-only mode
        absorbing: isReadOnly,  // If read-only, this will block any gestures
        child: Container(
          width: 100,
          height: 50,
          decoration: BoxDecoration(
            color: selectedMeal == meal
                ? Color.fromARGB(255, 133, 169, 143)
  ...
  Future<void> _AddMeal() async {
    try {
      final String result = await platform.invokeMethod('UpdateMeal', {
        "userId": uid,
        "meal": selectedMeal,
        "date": date.text,
        "oldRecord": oldRecord,
  ...
  Future<void> _getNutrientsData() async {
    final data = await CalorieService.fetchNutrientsData(uid!, dateD, title, mealTitle);

    if (data == null) {
      print("Error: Unable to fetch nutrients data");
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Error: Unable to fetch nutrients data")),
      );
    } else {
      setState(() {
        carb = data['carb'];
...
class CalorieService {
  static const MethodChannel _channel = MethodChannel('AddMeal');

  // Method to fetch calorie data
  static Future<Map<String, dynamic>?> fetchNutrientsData(String userId, String date, String meal, String food) async {
    try {
